...
opaque parseFromRust : String → String → String → Array String
...
-- def convert (str: Array String): Array Triple := str[0]

-- def parse (a b c: String) := convert $ parseFromRust a b c

def main : IO Unit :=
  -- IO.println $ addFromRust "Hello from Lean!"
  IO.println $ parseFromRust "<http://example.org/a> <http://example.org/b> \"c\", \"d\", \"f\", [] ." "text/turtle" ""
